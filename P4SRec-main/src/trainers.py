...
from modules import NCELoss, NTXent, info_nce
...
class Trainer:
    def __init__(self, model, train_dataloader,
        ...
        #self.cf_criterion = info_nce(self.args.temperature, self.device)
        print("self.cf_criterion:", self.cf_criterion.__class__.__name__)

        
    def train(self, epoch):
        # switch to augmentation method
        if self.args.train_strategy == 'R-S':
            all = False
            print('First stage: switch to random augmentation method')
            if epoch > self.args.augmentation_method_epoches:
                all = True
                print('Second stage: switch to simliar augmentation method')
        elif self.args.train_strategy == 'S-R':
            all = True
            print('First stage: switch to similar augmentation method')
            if epoch > self.args.augmentation_method_epoches:
                all = False
                print('Second stage: switch to random augmentation method')
        elif self.args.train_strategy == 'Alt-SR':
            all = True
            print('First: switch to similar augmentation method')
            if (epoch // self.args.augmentation_method_epoches) % 2 == 0:
                all = False
                print('Second: switch to random augmentation method')
        elif self.args.train_strategy == 'S':
            all = True
            print('Use to similar augmentation method')
        elif self.args.train_strategy == 'R':
            all = False
            print('Use to random augmentation method')

        user_seq, rating_seq, _, _, _ = get_user_seqs(self.args.data_file)
        self.train_dataset = RecWithContrastiveLearningDataset(self.args, user_seq[:int(len(user_seq)*self.args.training_data_ratio)], rating_seq[:int(len(rating_seq)*self.args.training_data_ratio)],
                                                      data_type='train', all=all)
        self.train_sampler = RandomSampler(self.train_dataset)
        self.train_dataloader = DataLoader(self.train_dataset, sampler=self.train_sampler, batch_size=self.args.batch_size)
...
class CoSeRecTrainer(Trainer):
    ...
    def iteration(self, epoch, dataloader, full_sort=True, train=True):
        ...
        if train:
            ...
            for i, (rec_batch, cl_batches, *rest) in rec_cf_data_iter:
                '''
                ...
                user_ids, input_ids, target_pos, target_neg, answers= rec_batch
