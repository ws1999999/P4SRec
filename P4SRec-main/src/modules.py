...
class info_nce(nn.Module):
    ...
    def forward(self, batch_sample_one, batch_sample_two):
        batch_size = batch_sample_one.size(0)
        N = 2 * batch_size
        z = torch.cat((batch_sample_one, batch_sample_two), dim=0)
        sim = torch.mm(z, z.T) / self.temperature
        sim12 = torch.diag(sim, batch_size)
        sim21 = torch.diag(sim, -batch_size)
        positive_samples = torch.cat((sim12, sim21), dim=0).reshape(N, 1)
        ...
        info_nce_loss = self.criterion(logits, labels)
