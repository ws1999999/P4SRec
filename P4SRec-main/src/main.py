...
def main():
    ...
    parser.add_argument('--data_name', default='Toys', type=str, help="Beauty, Sports, Toys")
    ...
    parser.add_argument('--augment_threshold', default=20, type=int, \
    ...
    parser.add_argument('--train_strategy', default='R', type=str, \
                        help="training strategy adustment method. Chosen from: \
                                S,R,R-S, S-R, Alt-SR.")
    parser.add_argument("--augmentation_method_epoches", type=float, default=240, \
                        help="number of epochs to switch from \
                        random to similarity model.")
    ...
    parser.add_argument("--tao", type=float, default=0.3, help="crop ratio for crop operator")
    ...
    # model args
    parser.add_argument("--model_name", default='P4SRec', type=str)
    ...
    user_seq, rating_seq, max_item, valid_rating_matrix, test_rating_matrix = \
    ...
    train_dataset = RecWithContrastiveLearningDataset(args, user_seq[:int(len(user_seq)*args.training_data_ratio)],
                                                      rating_seq[:int(len(rating_seq)*args.training_data_ratio)],
                                                      data_type='train')
    train_sampler = RandomSampler(train_dataset)
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.batch_size)

    eval_dataset = RecWithContrastiveLearningDataset(args, user_seq,rating_seq, data_type='valid')
    eval_sampler = SequentialSampler(eval_dataset)
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.batch_size)

    test_dataset = RecWithContrastiveLearningDataset(args, user_seq, rating_seq, data_type='test')
    ...
    trainer = CoSeRecTrainer(model, train_dataloader, eval_dataloader, test_dataloader, args)
