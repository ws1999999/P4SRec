...
def get_user_seqs(data_file):
    ...
    return user_seq, rating_seq, max_item, valid_rating_matrix, test_rating_matrix


def get_user_seqs_long(data_file):
    item2rating = json.loads(open(data_file).readline())
    user_seq = []
    long_sequence = []
    item_set = set()
    for user, items in item2rating.items():
        items = [int(item) for item in items.keys()]
...
def _load_train_data_list(data_file=None):
    train_data = []
    train_data_list = []
    train_data_set_list = []
