...
class OfflineItemSimilarity:
    def __init__(self, data_file=None, similarity_path=None, model_name='ItemCF', \
        ...
        self.train_data_list, self.train_item_list, self.train_data = self._load_train_data(data_file)
    ...
    def _load_train_data(self, data_file=None):
        """
        read the data from the data file which is a JSON dataset
        """
        train_data_list = []
        train_data_set_list = []
        ...
        for user, items in data.items():
            ...
            for item, rating in items.items():
                user_items.append((item, int(rating)))
            train_data_list.append(user_items[:-3])  # 删除每项的最后三个项
        for user_items in train_data_list:
            for item, _ in user_items:
                train_data_set_list.append(item)
        # 去除重复项，保留唯一的项
        train_data_set_list = list(set(train_data_set_list))
        train_data = {}
        for key, value in data.items():
            # 获取每个项的键值对，并删除最后三项
            items_to_keep = list(value.items())[:-3]
            # 将删除最后三项的键值对添加到train_data字典中
            train_data[key] = dict(items_to_keep)
    ...
    def _generate_item_similarity(self,train=None, save_path='./'):
        ...
        if self.model_name in ['ItemCF', 'ItemCF_IUF']:
            ...
            for idx, (cur_item, related_items) in c_iter:
                ...
                # 计算杰卡德相似系数
                    jac_sim = C[cur_item][related_item] / math.sqrt(N[cur_item] * N[related_item])

                    if self.model_name == 'ItemCF':
                    # 使用杰卡德相似系数，并将皮尔逊相关系数的绝对值作为权重
                        pearson_weight = abs(C[cur_item][related_item] / math.sqrt(N[cur_item] * N[related_item]))
                        self.itemSimBest[cur_item].setdefault(related_item, 0)
                        self.itemSimBest[cur_item][related_item] = jac_sim * pearson_weight

                    elif self.model_name == 'ItemCF_IUF':
                    # 使用杰卡德相似系数，并将皮尔逊相关系数的绝对值作为权重，并乘以降低热门物品影响的项
                        iuf_weight = score / math.sqrt(N[cur_item] * N[related_item])
                        pearson_weight = abs(C[cur_item][related_item] / math.sqrt(N[cur_item] * N[related_item]))
                        self.itemSimBest[cur_item].setdefault(related_item, 0)
                        self.itemSimBest[cur_item][related_item] = jac_sim * pearson_weight * iuf_weight
        # 保存相似度字典
            ...
            # 将评分信息整合到训练数据中
            train_data_with_ratings = []
            for item_ratings in self.train_data_list:
                item_data = [item_rating[0] for item_rating in item_ratings]
                item_data.extend(str(item_rating[1]) for item_rating in item_ratings)
                train_data_with_ratings.append(item_data)
            item2vec_model = Word2Vec(sentences=train_data_with_ratings, vector_size=20, window=5,
                                      min_count=1, epochs=100)
            print('train_data_with_ratings:',train_data_with_ratings)
            self.itemSimBest = dict()
...
if __name__ == '__main__':
    ...
    print('similiar_items:',similiar_items)